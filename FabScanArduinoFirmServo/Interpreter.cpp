...
void help() {
  ...
  Serial.println(F("G05 [D(grados)]; - apunta Ojo poner espacio entre D y dir"));
  ...
  Serial.println(F("M300; - Informa los baudios de comunicacion serie"));
}
...
void processCommand() {
    ...
    case 5:
      ...
      //Despues de G5 poner Ddir para apuntar a dir
      dir = parsenumber('D',87); // 87 es la mejor direccion en mi fabscan
      //output("Float",dir);
      val= int(dir) % 180;
      //output("Int",val);
...
/**
 * imprime los baudios de comunicacion programada
 */
void baudios(){
  output("B",float(BAUD_RATE));
    Serial.println("BAUDIOS"+ String(BAUD_RATE));
